#!/bin/bash
# oexctl Installer
# Installation: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/xmazu/openenvx/main/runtime/scripts/install.sh)"
...
main() {
    ...
    if [ -x "${INSTALL_DIR}/${BINARY_NAME}" ]; then
        ...
        if [ "$INSTALLED_VERSION" = "$VERSION" ]; then
            print_success "oexctl v${VERSION} is already installed at ${INSTALL_DIR}/${BINARY_NAME}"
            echo "" >&2
            print_info "Next steps:"
            echo "  Run an app: oexctl proxy run myapp -- npm run dev" >&2
            echo "  Access at:  https://myapp.localhost:1355" >&2
            exit 0
        fi
    fi

    print_info "Downloading oexctl v${VERSION}..."
    ...
    print_success "Installed oexctl v${VERSION} to ${INSTALL_DIR}/${BINARY_NAME}"
    ...
    echo "  Access at:  https://myapp.localhost:1355" >&2
    echo "" >&2
    echo "For more info: oexctl --help" >&2
}
